"""Forward operators A and additive noise n for measurement y = A x + n."""
...
from utils.utils_sisr import pre_calculate_FK, pre_calculate_nonuniform
...
class LinearOperator(ABC):
    ...
    def A(self, data, **kwargs):
        # Alias for forward method (for FlowDPS compatibility)
        return self.forward(data, **kwargs)

    def At(self, data, **kwargs):
        # Alias for transpose method (for FlowDPS compatibility)
        return self.transpose(data, **kwargs)

    def A_pinv(self, data, **kwargs):
        # Pseudo-inverse - default to transpose, subclasses can override
        return self.transpose(data, **kwargs)
...
# Operator-name aliases (accepted in task configs as alternatives to the
# canonical names declared above).

@register_operator(name='super_resolution')
class SuperResolutionOperatorAlias(SuperResolutionOperator):
    pass

@register_operator(name='motion_blur')
class MotionBlurOperatorAlias(MotionBlurOperator):
    pass

@register_operator(name='gaussian_blur')
class GaussianBlurOperatorAlias(GaussialBlurOperator):
    pass
